...
public class SearchServer {
    ...
    private void makeRunTimeSuggestion(String code, List<Mapping> completeCodeResultList) {
        ...
        if (suggestionLoL != null && !suggestionLoL.isEmpty()) {
            // code is start over, clear the stack. The composition is start over.
            // Jeremy'15,6,4.
            if (code.length() == 1) {
                ...
                if (bestSuggestionStack != null && !bestSuggestionStack.isEmpty()
                        && bestSuggestionStack.lastElement().second.equals(lastCode)) {
        ...
        if (DEBUG || dumpRunTimeSuggestion)
            Log.i(TAG,
                    "makeRunTimeSuggestion(): Finish checking for the composing is start over or user pressed backspace. Time elapsed  = "
                            + (System.currentTimeMillis() - startTime));

        // 15,6,8 Jeremy. Check exact match records first.
        if (completeCodeResultList != null && !completeCodeResultList.isEmpty()
                && completeCodeResultList.get(0).isExactMatchToCodeRecord()) {
            ...
            do {
                ...
                if (DEBUG || dumpRunTimeSuggestion)
                    Log.i(TAG, "makeRunTimeSuggestion() complete code = " + code + "" +
                            ...
                            + " score =" + exactMatchMapping.getScore() + ", bases core="
                            + exactMatchMapping.getBasescore()
                ...
                if (exactMatchMapping.getBasescore() > 0) {
                    if (k == 0 && exactMatchMapping.getWord().length() > 1) { // clear all previous traces if exact
                                                                              // match phrase found
                    ...
                    // trace back to mappings in snapshot if the exact matching word is start with
                    // it.
                    if (suggestLoLSnapshot != null) {
                        for (int i = 0; i < suggestLoLSnapshot.size(); i++) {
                            if (suggestLoLSnapshot.get(i) != null && !suggestLoLSnapshot.get(i).isEmpty()
                                    && exactMatchMapping.getWord()
                                            .startsWith(suggestLoLSnapshot.get(i).get(0).first.getWord())) {
                                ...
                                if (suggestLoLSnapshot.get(i).size() > 1) {
                                    for (int j = 1; j < suggestLoLSnapshot.get(i).size(); j++) {
                                        if (exactMatchMapping.getWord()
                                                .startsWith(suggestLoLSnapshot.get(i).get(j).first.getWord()))
                ...
                k++;
                if (DEBUG || dumpRunTimeSuggestion)
                    Log.i(TAG, "makeRunTimeSuggestion(): Check  " + k + "th exact match records. Time elapsed  = "
                            + (System.currentTimeMillis() - startTime));

            } while (completeCodeResultList.size() > k && completeCodeResultList.get(k).isExactMatchToCodeRecord()
                    && k < 5); // process at most 5 exact match items.
            ...
            if (!suggestionLoL.isEmpty() && highestScoreIndex != suggestionLoL.size() - 1) {// move bestSuggestionList
                                                                                            // to the last element
            ...
            if (DEBUG || dumpRunTimeSuggestion)
                Log.i(TAG, "makeRunTimeSuggestion() no exact match on complete code = " + code + ", time elapsed = "
                        + (System.currentTimeMillis() - startTime));

            /*
             * // if confirmed best suggestion found and contains last confirmed best
             * suggestion (double confirm) remove all other list not start with last
             * confirmed best suggestion
             * if( lastConfirmedBestSuggestion!=null && confirmedBestSuggestion!=null
             * &&lastConfirmedBestSuggestion.length()>1 &&
             * confirmedBestSuggestion.startsWith(lastConfirmedBestSuggestion)){
             * Iterator<List<Pair<Mapping,String>>> it = suggestionLoL.iterator();
             * while (it.hasNext()) {
             * List<Pair<Mapping,String>> item = it.next();
             * if(item.isEmpty()|| !item.get(item.size()-1).first.getWord().startsWith(
             * lastConfirmedBestSuggestion)){
             * it.remove();
             * }
             * }
             * }
            ...
            // iterate all previous exact match mapping and check for exact match on
            // remaining code.
            ...
            for (List<Pair<Mapping, String>> suggestionList : suggestionLoLSnapShot) {
                ...
                for (Pair<Mapping, String> p : suggestionList) {
                    ...
                    if (pCode.length() < code.length() && code.startsWith(pCode)
                            && code.length() - pCode.length() <= maxCodeLength) {
                        String remainingCode = code.substring(pCode.length());
                        if (DEBUG || dumpRunTimeSuggestion)
                            Log.i(TAG, "makeRunTimeSuggestion() working on previous exact match item = "
                                    + p.first.getWord() +
                                    " with base score = " + p.first.getBasescore() + ", average score = "
                                    + p.first.getBasescore() / p.first.getWord().length() +
                        ...
                        if (DEBUG || dumpRunTimeSuggestion)
                            Log.i(TAG,
                                    "makeRunTimeSuggestion() finish query on previous exact match item = "
                                            + p.first.getWord() +
                                            " , time elapsed =" + (System.currentTimeMillis() - startTime));

                        if (resultList.size() > 0
                                && resultList.get(0).isExactMatchToCodeRecord()) { // remaining code search got exact
                                                                                   // match
                            ...
                            if (DEBUG || dumpRunTimeSuggestion)
                                Log.i(TAG, "makeRunTimeSuggestion() remaining code = " + remainingCode + "" +
                                        ", got exact match  = " + remainingCodeExactMatchMapping.getWord()
                                        + " with base score = "
                                        + remainingScore + " average score =" + averageScore + " , highestScoreIndex = "
                                        + highestScoreIndex + ", time elapsed ="
                                        + (System.currentTimeMillis() - startTime));
                                ...
                                if (DEBUG || dumpRunTimeSuggestion)
                                    Log.i(TAG,
                                            "makeRunTimeSuggestion()  run-time suggest phrase verified from related table ="
                                                    ...
                                                    + " " +
                                                    ", new average score = " + highestScore + " , highestScoreIndex = "
                                                    + highestScoreIndex + ", time elapsed ="
                                                    + (System.currentTimeMillis() - startTime));
                ...
                i++;
                if (DEBUG || dumpRunTimeSuggestion)
                    Log.i(TAG, "makeRunTimeSuggestion() : remaing cod search +" + i + "th run.  time elapsed = "
                            + (System.currentTimeMillis() - startTime));
            }
            if (!suggestionLoL.isEmpty() && highestScoreIndex != suggestionLoL.size() - 1) {// move bestSuggestionList
                                                                                            // to the last element
         ...
         * if (bestSuggestionStack != null && !bestSuggestionStack.isEmpty() &&
         * bestSuggestionStack.size() > 1) {
         * for (int i = bestSuggestionStack.size() - 1; i > 0; i--) {
         * if (code.length() - bestSuggestionStack.get(i).first.getCode().length() >
         * maxCodeLength) {
         * String lastBestSuggestion = bestSuggestionStack.get(i - 1).first.getWord(),
         * bestSuggestion = bestSuggestionStack.get(i).first.getWord();
         * if (lastBestSuggestion != null &&
         * lastBestSuggestion.length() > 1 && bestSuggestion.length() >=
         * lastBestSuggestion.length()) {
         ...
         * Log.i(TAG, "makeRunTimeSuggestion() last confirmed best suggestion = " +
         * lastConfirmedBestSuggestion);
         * if (confirmedBestSuggestion != null)
         * Log.i(TAG, "makeRunTimeSuggestion() confirmed best suggestion = " +
         * confirmedBestSuggestion);
         * if (!bestSuggestionStack.isEmpty()) {
         * int i = 0;
         * for (Pair<Mapping, String> it : bestSuggestionStack) {
         * Log.i(TAG, "makeRunTimeSuggestion() best suggestion stack (" + (i) + ")= " +
         * bestSuggestionStack.get(i).first.getWord());
         * i++;
         * }
         * }
         * }
            ...
            for (int i = 0; i < suggestionLoL.size(); i++) {
                if (suggestionLoL.get(i) != null && !suggestionLoL.get(i).isEmpty()) {
                    for (int j = 0; j < suggestionLoL.get(i).size(); j++) {
                        // Log.i(TAG, "makeRunTimeSuggestion() suggestionLoL(" + i + ")(" + j + "):
                        // word="
                        // + suggestionLoL.get(i).get(j).first.getWord() + ", code=" +
                        // suggestionLoL.get(i).get(j).second
                        // + ", base score=" + suggestionLoL.get(i).get(j).first.getBasescore()
                        // + ", average base score=" + suggestionLoL.get(i).get(j).first.getBasescore()
                        // / suggestionLoL.get(i).get(j).first.getWord().length()
                        // + ", score=" + suggestionLoL.get(i).get(j).first.getScore());
                    }
                }
            }

            // Log.i(TAG,"makeRunTimeSuggestion() time elapsed = " +
            // (System.currentTimeMillis()- startTime ) );
        }
    }
