...
public class LIMEBaseKeyboard {
    ...
    /**
     * Total width of the keyboard, including left side gaps and keys, but not any
     * gaps on the
     * right side.
     */
    ...
    /**
     * Reserved space in the middle in unit of columns for separated keyboard in
     * landscape mode.
     */
     ...
     * Jeremy '26,1,7: Changed from 1.2f to 1.0f to prevent layout overflow (10 keys
     * * 1.2 = 120% width).
     */
     ...
     * Y-coordinate.
     * Some of the key size defaults can be overridden per row from what the
     * {@link LIMEBaseKeyboard}
     * defines.
     *
     * @attr ref R.styleable#Keyboard_keyWidth
     * @attr ref R.styleable#Keyboard_keyHeight
    ...
    public static class Row {
         ...
         * {@link LIMEBaseKeyboard#EDGE_TOP EDGE_TOP} and
         * {@link LIMEBaseKeyboard#EDGE_BOTTOM EDGE_BOTTOM}
         */
    ...
    public static class Key {
         ...
         * {@link LIMEBaseKeyboard#EDGE_LEFT}, {@link LIMEBaseKeyboard#EDGE_RIGHT},
         * {@link LIMEBaseKeyboard#EDGE_TOP} and
         ...
         * @param parent the row that this key belongs to. The row must already be
         *               attached to
        ...
        public Key(Resources res, Row parent, int x, int y, XmlResourceParser parser) {
            ...
            width = getDimensionOrFraction(a,
                    R.styleable.LIMEBaseKeyboard_keyWidth,
                    keyboard.mDisplayWidth, Math.round((float) (parent.defaultWidth * keyWidthScale)), keyWidthScale); // Jeremy
                                                                                                                       // '12,5,26
         ...
         * @return whether or not the point falls inside the key. If the key is attached
         *         to an edge,
         *         it will assume that all points between the key and the edge are
    ...
    /**
     * <p>
     * Creates a blank keyboard from the given resource file and populates it with
     * the specified
     * characters in left-to-right, top-to-bottom fashion, using the specified
     * number of columns.
     * </p>
     * <p>
     ...
     * @param characters          the list of characters to display on the keyboard.
     *                            One key will be created
     *                            for each character.
     * @param columns             the number of columns of keys to display. If this
     *                            number is greater than the
     *                            number of keys that can fit in a row, it will be
     *                            ignored. If this number is -1, the
     *                            keyboard will fit as many keys as possible in each
     *                            row.
     */
    public LIMEBaseKeyboard(Context context, int layoutTemplateResId,
        ...
        this(context, layoutTemplateResId, keySizeScale, 0, 0); // Jeremy '12,5,21 never show arrow keys in popup
                                                                // keyboard
        int x = 0;
        int y = 0;
        int column = 0;
     ...
     * @param x the x-coordinate of the point
     * @param y the y-coordinate of the point
     * @return the array of integer indices for the nearest keys to the given point.
     *         If the given
    ...
    protected int createArrowKeys(Resources res, int x, int y, boolean verticalLayout) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            // Cross shape arrow keys layout if center reserved space is larger than 2 ,
            // Jeremy '12,5,28
    ...
    private void loadKeyboard(Context context, XmlResourceParser parser) {
            ...
            while ((event = parser.next()) != XmlResourceParser.END_DOCUMENT) {
                if (event == XmlResourceParser.START_TAG) {
                    ...
                    if (TAG_ROW.equals(tag)) {
                        ...
                        // repeat space keys or keys longer then the distance between space key and half
                        // of screen on right keyboard.
                        if (DEBUG)
                        ...
                        if (mSplitKeyboard && leftSplitBorder > 0
                                && ((key.x >= leftSplitBorder && key.x < centerLine) // key left bound in between split
                                                                                     // border and centerline
                                        || (key.x < leftSplitBorder // the key right bound is too closed to centerline
                                                                    // so as no enough clearance for center arrow keys
                            ...
                            if (keyRightBound > centerLine + key.gap + mSplitKeyWidth + reservedCenterSpace) {
                                ...
                                final Key rightKey = new Key(currentRow, key); // clone the space key for the space key
                                                                               // on right keyboard.
                                rightKey.x = centerLine + key.gap + reservedCenterSpace;
                                rightKey.width = keyRightBound - rightKey.x;
                                mKeys.add(rightKey);
                                x += rightKey.gap * 2 + rightKey.width + reservedCenterSpace * 2; // shift x for the
                                                                                                  // distance on center
                                                                                                  // reserved space +
                                                                                                  // right key width
                            }
                        ...
                        if (mSplitKeyboard) {
                            leftSplitBorder = (mKeysInRow / 2 - 1) * mDefaultHorizontalGap
                                    + (mKeysInRow / 2) * mSplitKeyWidth;
                            ...
                            if (DEBUG)
                                Log.i(TAG,
                                        "loadkeyboard() keyboard attributed parsed, leftSplitBorder = "
                                                + leftSplitBorder
        ...
        if (DEBUG)
            Log.i(TAG, "loadKeyboard():mTotalHeight" + mTotalHeight);
    }
    ...
    static int getDimensionOrFraction(TypedArray a, int index, int base, int defValue, float scale) {
        ...
        if (value.type == TypedValue.TYPE_DIMENSION) {
            // Log.i(TAG, "getDimensionOrFraction() got dimension value, defvalue = " +
            // defValue + ". scale = " +scale);
            return (int) (a.getDimensionPixelOffset(index, defValue) * scale); // Jeremy '11,9,4
        } else if (value.type == TypedValue.TYPE_FRACTION) {
            // Round it to avoid values like 47.9999 from getting truncated
            // Log.i(TAG, "getDimensionOrFraction() got fraction value, base = " + base + ".
            // defvalue = " + defValue + ". scale = " +scale);
            return (int) (a.getFraction(index, base, base, defValue) * scale); // Jeremy '12,5,26 add scale. '12,5,27
                                                                               // use (int) instead of round to avoid
                                                                               // rouding error
        }
        return defValue;
    }
