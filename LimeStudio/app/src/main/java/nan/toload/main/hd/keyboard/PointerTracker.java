...
import nan.toload.main.hd.keyboard.LIMEKeyboardBaseView.OnKeyboardActionListener;
import nan.toload.main.hd.keyboard.LIMEKeyboardBaseView.UIHandler;
...
public class PointerTracker {
    ...
    // true if event is already translated to a key action (long press or
    // mini-keyboard)
    ...
    public void onDownEvent(int x, int y, long eventTime) {
        ...
        if (mListener != null) {
            if (isValidKeyIndex(keyIndex)) {
                ...
                // at
                // {@link #setKeyboard}. In those cases, we should update keyIndex according to
                // the
                // new keyboard layout.
    ...
    public void repeatKey(int keyIndex) {
        Key key = getKey(keyIndex);
        if (key != null) {
            // While key is repeating, because there is no need to handle multi-tap key, we
            // can
    ...
    private void showKeyPreviewAndUpdateKey(int keyIndex) {
        ...
        updateKey(keyIndex);
        // The modifier key, such as shift key, should not be shown as preview when
        // multi-touch is
        // supported. On the other hand, if multi-touch is not supported, the modifier
        // key should
        // be shown as preview.
    ...
    private void startLongPressTimer(int keyIndex) {
        ...
        // We use longer timeout for sliding finger input started from the symbols mode
        // key.
    ...
    private void detectAndSendKey(int index, int x, int y, long eventTime) {
        ...
        if (key == null) {
            ...
            if (key.text != null) {
                ...
                /*
                 * Swap the first and second values in the codes array if the primary code is
                 * not
