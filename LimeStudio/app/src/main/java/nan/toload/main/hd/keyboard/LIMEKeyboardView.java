...
public class LIMEKeyboardView extends LIMEKeyboardBaseView {
	...
	protected boolean onLongPress(Key key) {
		...
		if (key.codes[0] == LIMEBaseKeyboard.KEYCODE_DONE) {
			...
			return true;
				...
				&& Math.abs(((LIMEKeyboard) this.getKeyboard()).getSpaceDragDiff()) < mKeyHeight / 5) { // Jeremy
																										// '12,4,23
																										// avoid small
																										// move blocking
																										// the long
	...
	public boolean onTouchEvent(@NonNull MotionEvent me) {
		if (DEBUG)
			Log.i(TAG, "OnTouchEvent(), me.getAction() =" + me.getAction());
		...
		if (me.getAction() == MotionEvent.ACTION_UP) {
			...
			if (DEBUG)
				Log.i(TAG, "OnTouchEvent(), ACTION_UP, spaceDragDirection:" + spaceDragDirection);
